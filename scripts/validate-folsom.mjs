...
assert.equal(KEEPERS_LANTERN_LIGHTING.point.distance, 7, 'Keeper lantern illumination is bounded to seven world units.');
assert.equal(KEEPERS_LANTERN_LIGHTING.point.decay, 2, 'Keeper lantern uses inverse-square attenuation.');
...
assert.equal(lanternViewmodel.lanternLight.castShadow, false, 'Lantern light stays mobile-friendly and shadowless.');
assert.equal(lanternViewmodel.lanternLight.distance, KEEPERS_LANTERN_LIGHTING.point.distance, 'Physical lantern body carries the authored bounded local light.');
assert.equal(lanternViewmodel.lanternLight.layers.mask, 1, 'Lantern light affects world geometry rather than only the viewmodel overlay.');
assert.equal(lanternViewmodel.root.children.some((child) => child.isSpotLight), false, 'Lantern has no second camera-attached spotlight fill.');
