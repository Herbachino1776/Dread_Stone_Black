...
for (const root of fishRoots) {
  ...
  if (![bodyLength, bodyHeight, bodyWidth].every(Number.isFinite)) fail(`${root.name} is missing finite body sanity dimensions.`);
  if (!(tail.position.x > -bodyLength * 0.5 && tail.position.x < -bodyLength * 0.35)) fail(`${root.name} tail does not overlap the rear body volume.`);
...
console.log(`Fish geometry sanity check passed for ${fishRoots.length} symmetrical volumetric fish.`);
