...
for(let i=0;i<=200;i+=1){const p=resolveOutdoorLightingProfile(i/200);assert.ok(p.fogNear<p.fogFar);assert.ok(p.keyIntensity>=0&&p.moonIntensity>=0);assert.ok(p.exposure>=0.7&&p.exposure<=1);}
const day=resolveOutdoorPresentationState(0),night=resolveOutdoorPresentationState(0.6);assert.ok(night.fogFar>=400);assert.ok(night.fogNear>=100);assert.equal(night.hemi,0);assert.equal(night.moonIntensity,0);assert.ok(night.fog.r<0.001&&night.fog.g<0.001&&night.fog.b<0.001);assert.equal(night.sunIntensity,0);assert.equal(night.sunCastsShadow,false);assert.equal(night.moonCastsShadow,false);assert.equal(night.ordinaryEmissiveScale,0);assert.equal(night.environmentIntensity,0);assert.equal(night.playerNaturalLightIntensity,0);assert.equal(night.cameraNaturalLightIntensity,0);assert.equal(night.fallbackExplorationLightEnabled,false);assert.ok(night.outdoorExposure<=day.outdoorExposure);
