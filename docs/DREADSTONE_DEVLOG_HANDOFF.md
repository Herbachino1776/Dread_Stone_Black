...
<!-- last_processed_sha: 50be89fde7dee619f7ebe9e3038e0c5881b5b286 -->
...
- **Six production Creature Pack sources are registered.** Chezwick, Dreadguard, Dread Ram God, Rusted Knight, Forest Knight, and White Knight have generated Creature Pack data. The three Rusted Warrior variants have conservative file-backed starter Creature Definitions created by the transactional intake workflow; their checked-in Forge artifacts previously recorded `PASS` validation evidence, which does not establish repository/build/device execution.
- **Creature Pack intake can make compatible humanoids inspectable without auto-promoting gameplay policy.** `scripts/lib/creature-lab-definition.mjs` and `register-creature-lab-definitions.mjs` create only a starter Creature Definition when no authoritative definition exists. Loadout, Enemy Preset, loot, AI, encounter placement, and production acceptance remain explicit later decisions.
- **Standalone GLB weapon intake and data-driven loadouts are implemented.** `IMPORT_WEAPON.cmd`, `scripts/import-weapon.mjs`, and `scripts/weapon-import-lib.mjs` install validated GLB 2.0 assets plus game-owned weapon JSON transactionally. `NpcWeaponRegistry` discovers file-backed weapon records and `NpcLoadout.js` derives deterministic humanoid loadouts; Forge Action/socket/class compatibility remains the final capability filter.
- **Creature Lab can author canonical Enemy Preset v2 records and promote them transactionally into the project.** The Lab retains body-height and weapon scale/grip/capsule calibration, stable preset ID/display name authoring, canonical serialization, and development-only Save-to-Project through the constrained preset installer/bridge. `EnemyPresetRegistry.js` production-discovers `src/game/creatures/presets/*.json`, with built-in Dread Ram God Great Mace retained only as fallback when no file-backed record owns that ID.
- **Commit `067a110` adds two concrete file-backed production Enemy Presets using the canonical Dreadstone mace loadout.** `src/game/creatures/presets/rusted_warrior_001_dreadstone_mace.json` defines “Rusted Knight — Dreadstone Mace” for Creature Definition `rusted_warrior_001`; `src/game/creatures/presets/rusted_warrior_001_white_knight_dreadstone_mace.json` defines “White Knight — Dreadstone Mace” for `rusted_warrior_001_white_knight`. Both use `humanoid_dreadstone_mace_main_hand`, target height `1.58602468`, asset scale `1`, grip position `[0,0,0]`, grip quaternion `[0.70710678,0,0,0.70710678]`, and attack capsule start `[0,0,-0.48]`, end `[0,0,-0.29]`, radius `0.13`.
- **The new preset commit does not add or alter loot, AI, encounter placement, Creature Pack technical data, weapon assets, runtime systems, tests, architecture documents, or audio.** The Forest Knight still has no file-backed Enemy Preset in the processed diff. The two new preset records also contain no `rewards` field, so no new preset-default Loot Profile is established by this commit.
- **Encounter installation already shares the authored preset catalog.** `scripts/encounter-installer-lib.mjs` includes file-backed Enemy Presets when validating encounter JSON, so these newly checked-in presets are eligible for the existing production/authoring preset-resolution path without a hand-maintained installer list.
...
- **Folsom currently has two checked-in canonical encounter definitions, `folsomtst` and `folsomfight`, both using `dread_ram_god_great_mace`.** No newly processed development commit adds the Rusted Knight or White Knight presets to an encounter or changes normal-game encounter selection/activation.
...
- **Validation evidence for this range is limited to committed data shape.** The reviewed development commit only adds two JSON preset files. No successful preset validation command, broader test suite, build, CI run, live-device run, or gameplay acceptance is established by its diff.
- **No new audio assets or audio runtime changes appear in the processed development commit.**
...
- **File-backed Enemy Presets are first-class production catalog entries.** The two new knight presets are canonical project data, not transient Lab state. Their presence still does not imply encounter placement, AI acceptance, loot tuning, or live gameplay acceptance.
- **Weapon/preset calibration is distinct from Forge capability.** The two new presets select the Dreadstone mace and provide explicit presentation/contact overrides, but runtime resolution must still reject an incompatible Creature Pack Action/socket/class chain rather than guessing capability.
- **Creature Lab authoring promotes canonical data rather than mutating registries in-memory.** Browser drafts remain convenience state; the development bridge accepts canonical preset JSON and the installer re-resolves production dependencies before writing.
- Existing Creature Definitions remain authoritative over generated starter definitions. Automatic Creature Pack registration does not create Enemy Presets, loot, AI, or encounter placement.
...
- Encounter construction/reset/disposal remains lifecycle-owned and transactional. Authoring filesystem access remains narrowly scoped to development endpoints and fixed project directories; no browser-selected arbitrary path, generic write endpoint, browser commit, or browser push is part of the architecture.

### Risks, inference, and next logical work

- **Verified production state:** Rusted Knight and White Knight now have file-backed Enemy Preset v2 records pairing their existing Creature Definitions with `humanoid_dreadstone_mace_main_hand` and explicit height/grip/contact calibration. Forest Knight remains at the starter Creature Definition stage.
- **Verified limit:** this range contains data promotion only. It does not prove the new presets successfully resolve their full Definition → Pack → Loadout → weapon → Forge Action/socket chain at runtime, nor that their dimensions/contact calibration are visually or physically accepted on device.
- **Verified limit:** neither new preset supplies `rewards.lootProfileId`; neither is placed in a canonical encounter. No new AI tuning, loot policy, spawn identity, encounter activation, or persistent world state was added.
...
- **Inference:** because the preset registry and encounter installer already discover file-backed presets, the two new knight records should become selectable through the existing authored-preset catalog after normal module/dev-server reload. This follows from previously implemented discovery behavior; it is not direct runtime proof from `067a110`.
- **Next logical work:** run focused Enemy Preset/Creature authoring validation, open both new presets in Creature Lab, verify body height, socketed mace pose, active capsule contact, and approved offensive Action execution on the intended Forge packs, then explicitly decide loot/AI tuning and place accepted variants through Encounter Authoring. Forest Knight still needs the same preset-promotion step if intended for production.

## Development History

### 2026-08-17 18:01 EDT — Update through `50be89f`

**Scanned range:** after canonical checkpoint `4dc568ce606a71e3fb6de9c4d50c5eb85f29c540` through observed `main` HEAD `50be89fde7dee619f7ebe9e3038e0c5881b5b286`. Commit `397a5f5` was ignored because its message begins with `docs(devlog):`. Merge commit `50be89f` was also excluded from development summarization because its inspected change is limited to `docs/DREADSTONE_DEVLOG_HANDOFF.md`; it is nevertheless the checkpoint because it is the observed `main` HEAD. One development commit was included.
...
1. **Rusted Knight and White Knight Dreadstone Mace production presets (`067a110`)**
   - Added `src/game/creatures/presets/rusted_warrior_001_dreadstone_mace.json` and `src/game/creatures/presets/rusted_warrior_001_white_knight_dreadstone_mace.json`, both canonical `dreadstone.enemy_preset.v2` records.
   - The Rusted Knight preset targets Creature Definition `rusted_warrior_001`; the White Knight preset targets `rusted_warrior_001_white_knight`. Both select `humanoid_dreadstone_mace_main_hand` and set target height `1.58602468`.
   - Both records provide the same weapon override: asset scale `1`, grip position `[0,0,0]`, grip quaternion `[0.70710678,0,0,0.70710678]`, and attack capsule start `[0,0,-0.48]`, end `[0,0,-0.29]`, radius `0.13`.
   - No `rewards` field, AI policy, encounter spawn, tests, validation artifact, runtime code, assets, architecture documentation, or audio change is part of this commit. No successful validation/build/CI/device result is inferred.
