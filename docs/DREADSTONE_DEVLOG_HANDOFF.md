...
<!-- last_processed_sha: a11425fae0c2edee79f0ccf9f3d25c0a0f4322c9 -->
...
- **The checked-in Folsom encounter catalog now contains two canonical definitions.** Existing `src/game/encounters/data/folsomtst.json` defines `folsomtst` / “FolsomTst” with one `dread_ram_god_great_mace` spawn at `[-2, 0.188, 5.75]`, yaw `0`, home radius `10`. New `src/game/encounters/data/folsomfight.json` defines `folsomfight` / “FolsomFight” with stable spawn `folsomfight_enemy_7725bfec57ec`, the same preset, position `[-1.85667731, 0.188, 12.71035722]`, yaw `2.9531555`, home radius `8`, and no explicit fixed-gold override.
...
- **Production encounter discovery is data-driven.** `EncounterRegistry.js` eagerly discovers encounter JSON files under `src/game/encounters/data/` through Vite, so valid installed encounters do not require separate JavaScript registry edits in the next normal session.
- **Mobile development UX is consolidated.** The persistent development entry is a compact `DEV` control/menu for Creature Lab, Encounter Authoring, and Combat Debug. Encounter Authoring uses a pointer-transparent top-edge rail, temporary drawers, and context-specific place/move/radius/test controls so ordinary world movement/look/tool controls remain available. Creature Lab no longer owns a separate fixed LAB button and opens directly when selected through the unified launcher.
- **Focused validation source exists for M9/M9.5 and combat systems.** M9 adds `tests/m9-encounters.test.mjs` / `validate:m9-encounters`; M9.5 adds `tests/m95-encounter-authoring.test.mjs` / `validate:m95-encounter-authoring`; Creature Lab coverage was updated for the unified launcher. Commit `a11425f` changes only encounter JSON and adds no test, runtime, build, CI, or device-validation evidence.
...
- **Verified production state:** the source catalog now contains two canonical Folsom Encounter Definitions, each containing one Dread Ram God Great Mace spawn. The normal-session encounter runtime can compose encounter enemies through physical armament, player damage, bounded combat brain, death, loot, and persistent player gold.
- **Verified limit:** commit `a11425f` only adds `folsomfight.json`; it does not modify runtime encounter selection/activation logic. The diff therefore does not establish whether `folsomfight` is intended to replace, supplement, or coexist live with `folsomtst` in a particular normal-game flow.
...
- **Verified validation state:** source includes M9/M9.5 focused suites and the locomotion regression assertion. **Not verified by this range:** successful `npm run validate:m9-encounters`, `npm run validate:m95-encounter-authoring`, `npm run validate:combat`, `npm run build`, CI, or live iPhone/deployed acceptance.
- **Inference:** with stable authored spawn identity, runtime encounter composition, project-installable authoring, and multiple checked-in Folsom definitions, the next architectural gap remains per-spawn persisted world state; the additional encounter also makes explicit encounter-selection/activation semantics increasingly important. This is inference from current boundaries, not evidence that persistence or selection work already exists.
- **Next logical work:** validate the checked-in `folsomtst` and `folsomfight` definitions through focused/aggregate checks and mobile Folsom acceptance; confirm the intended normal-game activation relationship between the two definitions; then define the smallest `spawnId`-keyed persistence boundary for dead/loot-claimed/resolved-reward state without moving wallet authority out of `PlayerCurrencyState`, runtime HP into encounter JSON, or Forge technical truth into game-authored records.
