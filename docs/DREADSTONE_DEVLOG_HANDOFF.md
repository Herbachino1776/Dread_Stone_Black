...
- **Current checked-in production creature packs do not yet expose live M6 armament capability.** The regenerated Chezwick, Dreadguard, and Dread Ram God descriptors explicitly normalize `attachmentSockets.available` and `offensiveActions.available` to `false` with diagnostics because their current Forge outputs lack those records. Existing packs therefore remain valid, but real authored animated-armament testing for those creatures requires new Forge exports; the runtime does not invent production offsets or attack timing.
...
- **Verified blocker:** Chezwick, Dreadguard, and Dread Ram God currently advertise no Forge attachment sockets or offensive Actions. The M6 runtime is present, but the repository cannot verify real animated armament on these production bodies until Animation Forge produces compatible re-exports.
