...
export class PerfDebugPanel {
  constructor({ game }) {
    ...
    this.toggles = { neckmen: query.get('neckmen') !== '0', neckmanActorsHidden: query.get('neckmanActorsHidden') === '1', neckmanStatic: query.get('neckmanStatic') === '1', neckmanAiOff: query.get('neckmanAiOff') === '1', neckmanFeudOff: query.get('neckmanFeudOff') === '1', neckmanBehaviorCollisionOff: query.get('neckmanBehaviorCollisionOff') === '1' || query.get('neckmanCollisionOff') === '1', neckmanCollisionOff: query.get('neckmanCollisionOff') === '1', neckmanTargetingOff: query.get('neckmanTargetingOff') === '1', neckmanMovementOff: query.get('neckmanMovementOff') === '1', neckmanCombatOff: query.get('neckmanCombatOff') === '1', neckmanStateMachineOff: query.get('neckmanStateMachineOff') === '1', neckmanRenderLite: query.get('neckmanRenderLite') === '1', neckmanPerfTrace: query.get('neckmanPerfTrace') === '1', ramHerd: query.get('ramHerd') !== '0', rammanStatic: query.get('rammanStatic') === '1', rammanAiOff: query.get('rammanAiOff') === '1', rammanActorsHidden: query.get('rammanActorsHidden') === '1', neckmanTargetRamMen: query.get('neckmanTargetRamMen') !== '0', foliage: true, shadows: true, gore: true, water: true, skybox: true, hud: true, lowDpr: false };
  ...
  addToggle(key) {
    const label = key === 'lowDpr' ? 'DPR cap' : key === 'neckmanBehaviorCollisionOff' ? 'behavior collision off' : key === 'neckmanCollisionOff' ? 'legacy collision off' : key;
  ...
  collect() {
    ...
    const locationLoad = this.game.sceneSessionHost?.getLocationLoadDebugSummary?.() ?? {};
  ...
  render(force = false) {
    ...
    const neckFlags = mobile?.perfDebugFlags ? Object.entries(mobile.perfDebugFlags).filter(([k, v]) => k.startsWith('neckman') || k.startsWith('ram') || k === 'neckmen').map(([k, v]) => `${k}=${v ? 'on' : 'off'}`).join(', ') : 'none';
    const trace = mobile?.perfTrace ?? {};
    const traceLine = mobile ? `Neckman modes: ${neckFlags}\nNeckman active: actors ${mobile.actorVisible ? 'yes' : 'no'} / mixers ${mobile.mixersActive ? 'yes' : 'no'} / AI ${mobile.aiActive ? 'yes' : 'no'} / feud ${mobile.feudManagerActive ? 'yes' : 'no'} / targeting ${mobile.targetingActive ? 'yes' : 'no'} / movement ${mobile.movementActive ? 'yes' : 'no'} / terrain grounding ${mobile.terrainGroundingActive ? 'yes' : 'no'} / behavior collision ${mobile.behaviorCollisionActive ? 'yes' : 'no'} / combat ${mobile.combatActive ? 'yes' : 'no'} / state ${mobile.stateMachineActive ? 'yes' : 'no'}\nNeckman ms: total ${(trace.subsystemMs ?? 0).toFixed?.(2) ?? 0} / scheduler ${(trace.schedulerMs ?? 0).toFixed?.(2) ?? 0} / manager ${(trace.managerMs ?? 0).toFixed?.(2) ?? 0} / enemies ${trace.enemyUpdateMs?.map?.((e) => `${e.id}:${e.ms.toFixed(2)}`).join(', ') || 'none'} / mixer ${(trace.mixerMs ?? 0).toFixed?.(2) ?? 0} / AI ${(trace.aiMs ?? 0).toFixed?.(2) ?? 0} / targeting ${(trace.targetingMs ?? 0).toFixed?.(2) ?? 0} / movement ${(trace.movementMs ?? 0).toFixed?.(2) ?? 0} / collision avg/worst ${(trace.collisionMs ?? 0).toFixed?.(2) ?? 0}/${(trace.collisionWorstMs ?? 0).toFixed?.(2) ?? 0} / combat ${(trace.combatMs ?? 0).toFixed?.(2) ?? 0} / state ${(trace.stateMachineMs ?? 0).toFixed?.(2) ?? 0} / debug ${(trace.debugMs ?? 0).toFixed?.(2) ?? 0}\nNeckman counters/s: AI run ${trace.aiUpdatesRunPerSecond ?? 0} / skipped ${trace.intentionallySkippedPerSecond ?? 0} / deferred ${trace.deferredAiPerSecond ?? 0} / dropped catch-up ${trace.droppedCatchUpPerSecond ?? 0} / actor sep ${trace.actorSeparationChecksPerSecond ?? 0} / obstacle ${trace.obstacleChecksPerSecond ?? 0} / combat prox ${trace.combatProximityChecksPerSecond ?? 0} / unstuck ${trace.unstuckPathCorrectionChecksPerSecond ?? 0} / collision deferred ${trace.collisionOperationsDeferredPerSecond ?? 0} / budget over ${trace.collisionBudgetExceededCount ?? 0} / pairs considered ${trace.creaturePairsConsideredPerSecond ?? 0} / pairs resolved ${trace.creaturePairsResolvedPerSecond ?? 0} / collision checks ${trace.collisionChecksPerSecond ?? 0} / combat checks ${trace.combatChecksPerSecond ?? 0} / movement ${trace.movementUpdatesPerSecond ?? 0} / target scans ${trace.targetScansPerSecond ?? 0} / target ops ${trace.targetingOpsPerSecond ?? 0} / target deferred ${trace.deferredTargetingOpsPerSecond ?? 0} / processed frame ${trace.enemiesProcessedThisFrame ?? 0} / budget over ${trace.budgetExceededFrames ?? 0}\nNeckman targeting: active ${mobile.targetingActive ? 'yes' : 'no'} / prey cache ${trace.targetCacheSize ?? 0} / predator cache ${trace.predatorTargetCacheSize ?? 0} age ${(trace.targetCacheAgeSeconds ?? 0).toFixed?.(2) ?? 0}s / ops frame ${trace.targetingOpsThisFrame ?? 0} / avg ${(trace.targetingRollingMs ?? trace.targetScanRollingMs ?? 0).toFixed?.(3) ?? 0}ms / worst ${(trace.targetingWorstMs ?? 0).toFixed?.(3) ?? 0}ms / candidates ${trace.targetCandidatesPerScan ?? 0}/scan / work ${trace.targetingWorkMode ?? 'unknown'} / burst frames ${trace.burstedTargetingFrames ?? 0} / targets ${mobile.targetSummary?.map?.((e) => `${e.id}->${e.target ?? 'none'}(${e.type ?? 'none'})`).join(', ') || 'none'}\nNeckman recent worst: frame ${(trace.recentWorst?.subsystemMs ?? 0).toFixed?.(2) ?? 0}ms age ${trace.recentWorst?.ageFrames ?? 0}f/${(trace.recentWorst?.ageSeconds ?? 0).toFixed?.(1) ?? 0}s / scheduler ${(trace.recentWorst?.schedulerMs ?? 0).toFixed?.(2) ?? 0} / manager ${(trace.recentWorst?.managerMs ?? 0).toFixed?.(2) ?? 0} / enemy ${(trace.recentWorst?.enemyMs ?? 0).toFixed?.(2) ?? 0} ${trace.recentWorst?.enemyId ?? ''} / targeting ${(trace.recentWorst?.targetingMs ?? 0).toFixed?.(2) ?? 0} / movement ${(trace.recentWorst?.movementMs ?? 0).toFixed?.(2) ?? 0} / collision ${(trace.recentWorst?.collisionMs ?? 0).toFixed?.(2) ?? 0} / combat ${(trace.recentWorst?.combatMs ?? 0).toFixed?.(2) ?? 0} / mixer ${(trace.recentWorst?.mixerMs ?? 0).toFixed?.(2) ?? 0} / state ${(trace.recentWorst?.stateMachineMs ?? 0).toFixed?.(2) ?? 0} / debug ${(trace.recentWorst?.debugMs ?? 0).toFixed?.(2) ?? 0}\n` : '';
    const ram = mobile?.ramManHerd;
    const ramLine = ram ? `RamMen: herd ${ram.enabled ? 'on' : 'off'} / spawned ${ram.spawned} / alive ${ram.alive} / dead ${ram.dead} / failed ${ram.failed} / asset ${ram.assetStrategy} / canonical ${ram.canonicalPath} / roots ${ram.actorRoots} / skinned ${ram.skinnedRoots} / mixers ${ram.mixers} / update ${(ram.updateMs ?? 0).toFixed?.(2) ?? 0}ms / AI ${ram.aiUpdatesPerSecond ?? 0}/s\n` : '';
    const toggleLine = `neckmen ${this.toggles.neckmen ? 'on' : 'off'}, ramHerd ${this.toggles.ramHerd ? 'on' : 'off'}, foliage ${this.toggles.foliage ? 'on' : 'off'}, shadows ${this.toggles.shadows ? 'on' : 'off'}, gore ${this.toggles.gore ? 'on' : 'off'}, water ${this.toggles.water ? 'on' : 'off'}, dprCap ${this.toggles.lowDpr ? 'on' : 'off'}`;
    ...
    const locationLoadLine = m.locationLoad ? `Location load: current ${m.locationLoad.currentLocationId ?? 'n/a'} / loaded ${(m.locationLoad.loadedLocationIds ?? []).join(',') || 'none'} / scene ${m.locationLoad.activeSceneDefinitionId ?? 'n/a'} / collision ${m.locationLoad.activeCollisionSourceId ?? 'n/a'} / creatures ${m.locationLoad.activeCreatureRuntimeLocationId ?? 'n/a'}\nOff-location: objects ${m.locationLoad.offLocationObjectsCount ?? 0} / collision ${m.locationLoad.offLocationCollisionEntriesCount ?? 0} / creatures ${m.locationLoad.offLocationCreaturesCount ?? 0} / routes ${m.locationLoad.routeRegistryLoaded ? 'yes' : 'no'} / lazy pending ${m.locationLoad.lazyLocationsPending ? 'yes' : 'no'}\n` : '';
    this.lastReport = `Location: ${m.location}\n${locationLoadLine}${mobileLine}${ramLine}${traceLine}FPS: ${m.currentFps.toFixed(0)} / avg ${m.avgFps.toFixed(0)} / worst ${m.worstMs.toFixed(1)}ms\nRenderer: ${m.calls} calls / ${m.tris} tris / ${m.geoms} geoms / ${m.textures} textures\nScene: ${m.objects} objects / ${m.meshes} meshes / ${m.skinned} skinned / ${m.transparent} transparent / ${m.materials} materials / ${m.lights} lights / ${m.shadows} shadows\nGameplay: ${m.activeEnemies} enemies / ${m.activeNeckmen} neckmen / ${m.goreCount} gore\nCreature anim: ${m.activeAnimationMixers} active mixers / ${m.loadedCreatureAnimationRoots} loaded roots / ${m.liveSkinnedRoots} live skinned roots / extra state roots ${m.extraStateRootsAlive ? 'yes' : 'no'}\nFolsom blood feud: ${bloodFeudLine}\nDPR: ${m.dpr.toFixed(2)}  Canvas: ${m.width}x${m.height}\nToggles: ${toggleLine}`;
