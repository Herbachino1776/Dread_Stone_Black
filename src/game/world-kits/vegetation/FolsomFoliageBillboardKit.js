...
function distanceToSegment(x, z, from, to) { const dx = to[0] - from[0]; const dz = to[1] - from[1]; const lengthSq = dx * dx + dz * dz; if (lengthSq <= 0.000001) return Math.hypot(x - from[0], z - from[1]); const t = Math.max(0, Math.min(1, ((x - from[0]) * dx + (z - from[1]) * dz) / lengthSq)); return Math.hypot(x - (from[0] + dx * t), z - (from[1] + dz * t)); }
function inZone(point, zone) { const [x, z] = point; if (zone.kind === 'pathCorridor' && Array.isArray(zone.points) && zone.points.length >= 2) { const halfWidth = (zone.width ?? 0) * 0.5; return zone.points.slice(0, -1).some((from, index) => distanceToSegment(x, z, from, zone.points[index + 1]) <= halfWidth); } if (zone.radius) return Math.hypot(x - zone.center[0], z - zone.center[1]) <= zone.radius; if (zone.radiusX && zone.radiusZ) return (((x - zone.center[0]) ** 2) / (zone.radiusX ** 2)) + (((z - zone.center[1]) ** 2) / (zone.radiusZ ** 2)) <= 1; if (zone.minX !== undefined) return x >= zone.minX && x <= zone.maxX && z >= zone.minZ && z <= zone.maxZ; return false; }
