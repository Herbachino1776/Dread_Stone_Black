...
export class OutdoorLightingDirector {
  ...
  update(player = null) {
    ...
    if (this.debugPanel) {
      ...
      const spotTarget = torch.spotlightTarget ?? {};
      const fillOrigin = torch.fillOrigin ?? {};
      this.debugPanel.textContent = `OUTDOOR ${state.name.toUpperCase()} phase ${state.phase.toFixed(4)}\nweights day/dusk/night/dawn ${state.dayWeight.toFixed(2)} ${state.duskWeight.toFixed(2)} ${state.nightWeight.toFixed(2)} ${state.dawnWeight.toFixed(2)}\nsun elev ${state.sunElevation.toFixed(3)} intensity ${state.sunIntensity.toFixed(3)} shadow ${this.key.castShadow}\nmoon ${state.moonIntensity.toFixed(3)} shadow false hemi ${state.hemi.toFixed(3)} environment ${state.environmentIntensity.toFixed(3)}\nfog #${state.fog.getHexString()} ${state.fogNear.toFixed(2)}-${state.fogFar.toFixed(2)} exposure ${state.outdoorExposure.toFixed(2)} emissive ${state.ordinaryEmissiveScale.toFixed(2)}\nTORCH equipped ${Boolean(torch.equipped)} lit ${Boolean(torch.lit)} active ${Boolean(torch.active)} mode ${torch.mode ?? 'exploration'}\nactor ${torch.activeActorId ?? 'none'} distance ${Number.isFinite(torch.actorDistance) ? torch.actorDistance.toFixed(2) : '-'}m facing ${Number.isFinite(torch.facingDot) ? torch.facingDot.toFixed(3) : '-'}\nspot origin ${(spotOrigin.x ?? 0).toFixed(2)},${(spotOrigin.y ?? 0).toFixed(2)},${(spotOrigin.z ?? 0).toFixed(2)} target ${(spotTarget.x ?? 0).toFixed(2)},${(spotTarget.y ?? 0).toFixed(2)},${(spotTarget.z ?? 0).toFixed(2)}\nspot angle ${THREE.MathUtils.radToDeg(torch.spotlightAngle ?? 0).toFixed(1)}deg penumbra ${(torch.spotlightPenumbra ?? 0).toFixed(2)} intensity ${(torch.spotlightIntensity ?? 0).toFixed(2)} range ${(torch.spotlightDistance ?? 0).toFixed(1)} decay ${torch.spotlightDecay ?? '-'} footprint ${(torch.spotlightFootprint ?? 0).toFixed(2)}m\npoint intensity ${(torch.intensity ?? 0).toFixed(2)} range ${(torch.range ?? 0).toFixed(1)} decay ${torch.decay ?? '-'}\nfill origin ${(fillOrigin.x ?? 0).toFixed(2)},${(fillOrigin.y ?? 0).toFixed(2)},${(fillOrigin.z ?? 0).toFixed(2)} intensity ${(torch.fillIntensity ?? 0).toFixed(2)} range ${(torch.fillDistance ?? 0).toFixed(1)} decay ${torch.fillDecay ?? '-'} layer ${torch.fillLayer ?? '-'}\ntorch shadow casters ${torch.shadowCasterCount ?? 0} map ${torch.shadowMapSize ?? 0}px near/far ${torch.shadowNear ?? '-'}-${torch.shadowFar ?? '-'} bias ${torch.shadowBias ?? '-'}/${torch.shadowNormalBias ?? '-'} self-shadow ${Boolean(torch.torchMeshSelfShadowStatus)}\nshadow lights ${activeShadowCasters} map ${this.shadowMapSize}px radius ${this.shadowRadius.toFixed(2)} (${this.combatShadowFocused ? 'combat' : 'exploration'}) texel ${texel.toFixed(4)}\nbias ${this.key.shadow.bias.toFixed(6)} normalBias ${this.key.shadow.normalBias.toFixed(4)} radius/filter ${this.key.shadow.radius}/PCFSoft\nACTIVE/AUTHORED LIGHT SOURCES\n${lightLines.join('\n') || '(none)'}`;
    }
    return this.debug;
  }
}
