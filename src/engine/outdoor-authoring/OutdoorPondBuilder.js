...
export function buildOutdoorPond(input) {
  ...
  const body = {
    ...
    shoreWidth: mudMargin + wetShoreWidth,
    fishable: Boolean(recipe.fishable),
    fishableRadius: recipe.fishable ? (recipe.fishableRadius ?? Math.max(recipe.size.radiusX, recipe.size.radiusZ) + mudMargin + wetShoreWidth + 3) : undefined,
    fishSpeciesPool: [...(recipe.fishSpeciesPool ?? [])],
    fishCatchSeed: `${recipe.seed ?? recipe.id}-catch`,
    footprint,
    pondDecor: {
      seed: recipe.seed,
