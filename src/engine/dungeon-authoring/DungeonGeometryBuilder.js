...
import { FISH_SPECS, createFishMesh } from '../../game/fishing/FishMeshFactory.js';
...
function addFishDisplay({ definition, group, primitive, materialFactory }) {
  ...
  const meshes = [];
  const materialResolver = (reference, fallback, { slot }) => makeMaterial(definition, primitive[slot] ?? reference, materialFactory, fallback);
  ...
  const fishRoot = createFishMesh(primitive.variant, { id: primitive.id, baseUserData: base, materialResolver });
