...
const OUTDOOR_CHEST_FIELDS = new Set(['id', 'label', 'position', 'itemId', 'acquiredMessage', 'rodVariant', 'bodyMaterial', 'strapMaterial', 'tags', 'metadata', 'userData']);
const FIELD_SURVIVAL_ITEM_IDS = new Set(['wood_axe', 'fishing_rod', 'flint_stick', 'torch', 'dreadstone_sword', 'dreadstone_mace']);
...
function validateOutdoorAuthoring(definition, errors, warnings) {
  ...
  if (terrain !== undefined) {
    ...
    asArray(terrain?.heightStamps).forEach((stamp, index) => {
      ...
      const outlineBased = stamp.kind === 'flattenOutline';
      ...
      if (outlineBased && !pointArrayIsFinite(stamp.outline, 3)) addIssue(errors, 'error', `heightStamp ${id} needs an outline with at least three finite [x, z] points`, id);
      ...
      if (['flatten', 'flattenOutline'].includes(stamp.kind) && !Number.isFinite(stamp.y)) addIssue(errors, 'error', `heightStamp ${id} y must be finite`, id);
      if (outlineBased && stamp.feather !== undefined && (!Number.isFinite(stamp.feather) || stamp.feather < 0)) addIssue(errors, 'error', `heightStamp ${id} feather must be finite and non-negative when present`, id);
