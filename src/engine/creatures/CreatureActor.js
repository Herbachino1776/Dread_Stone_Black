...
export class CreatureActor {
  ...
  refreshUserData(controllerState = {}) {
    ...
    this.group.userData = {
      ...
      animationStrategy: this.animationSet.getAssetStrategy?.() ?? (this.singleActorRoot ? 'singleActorRoot-extracted-clips' : 'legacy-separate-roots'),
      canonicalModelPath: this.animationSet.getCanonicalPath?.() ?? null,
      canonicalAnimationError: this.animationSet.canonicalError?.message ?? null,
